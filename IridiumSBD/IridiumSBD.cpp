...
int IridiumSBD::internalBegin()
{
   ...
   uint32_t startupTime = 500; //ms
    for (uint32_t start = millis(); millis() - start < startupTime;) {
        printf("millis = %lu, delta = %lu, start = %lu\n", millis(), millis()-start, start);
        printf("1st for loop with millis\n");
        time_t seconds = time(NULL);
        printf("RTC seconds = %s\n", ctime(&seconds));
        if (cancelled()) {
...
bool IridiumSBD::waitForATResponse(char *response, int responseSize, const char *prompt, const char *terminator)
{
   ...
   for (unsigned long start=millis(); millis() - start < 1000UL * atTimeout;)
   {
      ...
      while (true) {
         int readResult = serial.getc(); // CAREFUL, we are not sure if getc actually returns values when no more data to be read. We might need to use Serial instead of Stream 
...
void IridiumSBD::console(const char *msg)
{
   ...
   if (this->pConsoleSerial) {
      printf("has console\n");
      pConsoleSerial->printf("pConsoleSerial->printf\n");
      pConsoleSerial->printf("%s\n",msg);
  }
  printf("exit console\n");
}
...
int IridiumSBD::doSBDRB(uint8_t *rxBuffer, size_t *prxBufferSize)
{
   ...
   uint8_t byte1; 
   uint8_t byte2;
   bool byte1Read = false;
   bool atLeastTwoBytes = false;
   while (millis() - start < 1000UL * atTimeout)
   {
      if (cancelled())
         return ISBD_CANCELLED;

      // mbed replacement for stream.available() >= 2
      // Alternative: consider calling serial.readable() twice
      atLeastTwoBytes = hasReadTwoBytes(serial, byte1, byte2, byte1Read);
      if (atLeastTwoBytes) break;
      //if (stream.available() >= 2) // original Arduino code
      //   break;
   }
   ...
   uint16_t checksum = 256 * byte1 + byte2; 
   console(F("[csum:"));
   console(checksum);
   console(F("]"));
