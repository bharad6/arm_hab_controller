...
bool IridiumSBD::waitForATResponse(char *response, int responseSize, const char *prompt, const char *terminator)
{
   ...
   for (unsigned long start=millis(); millis() - start < 1000UL * atTimeout;)
   {
      ...
      while (true) {
         int readResult = serial.getc(); // CAREFUL, we are not sure if getc actually returns values when no more data to be read. We might need to use Serial instead of Serial
